...
const Header = () => {
  ...
  if (!user) {
    ...
    if (user.rol === roles.BUYER) {
      options = `<a href='/binary-shop/'><button class="baseBtn">Inicio</button></a><a href='/binary-shop/update-user.html'><button class="baseBtn">Actualizar perfil</button></a><a href='/binary-shop/cart.html'><button class="baseBtn">Ir al carrito</button></a><button class="baseBtn" id="logout">Cerrar sesión</button>`;
    }
